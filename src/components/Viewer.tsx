...
export const Viewer: React.FC<ViewerProps> = ({ 
  ...
  if (error) {
    ...
    <div className={`flex-1 flex flex-col max-w-7xl mx-auto w-full ${isReadingMode ? 'reading-mode' : ''}`}>
      ...
      {/* Mobile Action Button (only in non-reading mode) */}
      {!isReadingMode && (
        <button
          onClick={() => setShowMobileActions(true)}
          className="fixed bottom-16 right-4 z-40 w-12 h-12 sm:hidden bg-blue-600 rounded-full shadow-lg flex items-center justify-center text-white"
      ...
      {showMobileActions && (
        <div className="fixed inset-0 z-50 sm:hidden" aria-modal="true">
          ...
          ></div>
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-800 rounded-t-xl shadow-lg p-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              <button
                ...
                className="flex flex-col items-center justify-center p-2 rounded-lg bg-gray-50 dark:bg-gray-700"
              >
                <span className="text-xl mb-1">{showSource ? '📄' : '📝'}</span>
                <span className="text-xs">{showSource ? 'View Doc' : 'Source'}</span>
              </button>
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-2 rounded-lg ${tableOfContents.length > 0 ? 'bg-gray-50 dark:bg-gray-700' : 'opacity-50 bg-gray-50 dark:bg-gray-700'}`}
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-2 rounded-lg bg-gray-50 dark:bg-gray-700 ${isFavorite ? 'text-yellow-600 dark:text-yellow-400' : ''}`}
              >
                <span className="text-xl mb-1">{isFavorite ? '★' : '☆'}</span>
